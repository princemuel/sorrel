...
impl<'a> SnowballEnv<'a> {
    ...
    pub fn get_current(self) -> Cow<'a, str> { self.current }

    pub fn set_current(&mut self, current: &'a str) { self.current = Cow::from(current); }

    pub fn set_current_s(&mut self, current: String) { self.current = Cow::from(current); }
    ...
    pub fn eq_s_b(&mut self, s: &str) -> bool {
        if (self.cursor - self.limit_backward) < s.len() as i32 {
            false
            // Check if cursor -s.len is a char boundary. if not well... return
            // false obv
    ...
    pub fn hop_checked(&mut self, delta: i32) -> bool { return delta >= 0 && self.hop(delta); }
    ...
    // range (inclusive) so the grouping is represented with 18 bits, plus 6 bits of
    // padding:
    //
    ...
    pub fn slice_del(&mut self) { self.slice_from("") }
    ...
    pub fn assign_to(&mut self) -> String { self.current[0..self.limit as usize].to_string() }
