...
static A_3: &[Among<Context>; 2] = &[Among("č", -1, 1, None), Among("dž", -1, 2, None)];
...
fn r_step1(env: &mut SnowballEnv<'_>, context: &mut Context) -> bool {
...
fn r_step2(env: &mut SnowballEnv<'_>, context: &mut Context) -> bool {
    'replab0: loop {
...
fn r_fix_conflicts(env: &mut SnowballEnv<'_>, context: &mut Context) -> bool {
    let mut among_var;
    env.ket = env.cursor;
    if (env.cursor - 3 <= env.limit_backward
        || env.current.as_bytes()[(env.cursor - 1) as usize] >> 5 != 3
        || ((2621472 >> (env.current.as_bytes()[(env.cursor - 1) as usize] & 0x1f)) & 1) == 0)
    {
        return false;
    }
...
fn r_fix_chdz(env: &mut SnowballEnv<'_>, context: &mut Context) -> bool {
    let mut among_var;
    env.ket = env.cursor;
    if (env.cursor - 1 <= env.limit_backward
        || (env.current.as_bytes()[(env.cursor - 1) as usize] != 141
            && env.current.as_bytes()[(env.cursor - 1) as usize] != 190))
    {
        return false;
    }
...
fn r_fix_gd(env: &mut SnowballEnv<'_>, context: &mut Context) -> bool {
    env.ket = env.cursor;
    if !env.eq_s_b(&"gd") {
        return false;
    }
